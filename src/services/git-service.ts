...
const hookTemplate = (hook: GitHook, config: FullConfig, nodeModulesDir?: string) => {
    const headerComment =
        "#!/bin/bash\n" +
        "# This file was automatically generated by hoks\n" +
        "# Do not edit this file manually\n";

    const resolvedNodeModules = nodeModulesDir
        ? `${nodeModulesDir}/node_modules/.bin`
        : "./node_modules/.bin";
    const resolvedHoksBin = nodeModulesDir
        ? `${nodeModulesDir}/node_modules/.bin/hoks`
        : "./node_modules/.bin/hoks";

    const runCommand = isDevelopment()
        ? `nr start --type ${hook} "$@"`
        : `export PATH="${resolvedNodeModules}:$PATH"\n ${resolvedHoksBin} --type ${hook} "$@"`;
...
const writeHook = async (hook: GitHook, config: FullConfig) => {
    ...
    const nodeModulesDir = !isDevelopment()
        ? await PathService.getNearestHoksPackageJson()
        : undefined;
    const template = hookTemplate(hook, config, nodeModulesDir);
